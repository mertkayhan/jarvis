...
import {
  deleteDefaultPersonality,
  getAvailableTools,
  getPersonality,
  setDefaultPersonality,
  updatePersonality,
} from "@/components/personalities/personality-actions";
...
export default function Page() {
  ...
  const { personality, personalityLoading } = usePersonalityDetails(
    params.id,
    user?.email
  );
  ...
  const [currentDocumentSelection, setCurrentDocumentSelection] = useState<
    string[]
  >([]);
  // TODO:
  const [currentQuestionPackSelection, setCurrentQuestionPackSelection] =
    useState<QuestionPack | null>(null);
  const [currentDocumentPackSelection, setCurrentDocumentPackSelection] =
  ...
  const updateMutation = useMutation(
    {
      mutationFn: async (personalityId: string) => {
        ...
        if (isDefault) {
          await setDefaultPersonality(
            user?.email as string,
            personality?.id as string
          );
      ...
      onError: (error) => {
        ...
        toast({
          title: "Failed to update personality",
          variant: "destructive",
        });
      },
    },
  ...
  return (
    <TooltipProvider>
      <div className="flex h-full">
        <div className="flex h-full flex-col border-slate-300 bg-background transition-all duration-300">
          <Sidebar
            highlight={false}
            showChatList={false}
            moduleName="jarvis"
            ...
            <div className="w-full h-full flex flex-col space-y-2">
              ...
              <div className="flex py-2 gap-x-2 items-center">
                ...
                <KnowledgeDropdown
                  ...
                  triggerButton={
                    <Button variant="outline">Attach knowledge</Button>
                  }
                />
                <KnowledgeMenu
                  userId={user?.email as string}
