...
export function ChatListItem({ chat, path, i, userId, id, dispatch }: ChatListItemProps) {
    return (
        <TooltipProvider>
            <Tooltip>
                    ...
                    <TooltipTrigger asChild>
                        ...
                        >
                            <div className="relative w-full flex flex-1" key={i}>
                                ...
                                >
                                    <h1
                                        className="max-w-20 md:max-w-40 text-xs md:text-sm font-medium capitalize text-slate-700 dark:text-slate-200 relative max-h-5 select-none truncate break-all"
                                    >
                                        {chat.title ?? "Untitled"}
                                    </h1>
                                </motion.div>
                            ...
                            <p className="hidden md:block text-xs text-slate-500 dark:text-slate-400">
                                {formatDistanceToNow(chat.updatedAt, { addSuffix: true, includeSeconds: true })}
                            </p>
                        </div>
                    </TooltipTrigger>
                </div>
