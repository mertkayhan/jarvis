...
import { createPack, ListDocumentPacksResp } from "./document-packs-actions";

interface NewDocumentPackProps {
    userId: string
}

export function NewDocumentPack({ userId }: NewDocumentPackProps) {
    const [name, setName] = useState("");
    const [description, setDescription] = useState("");
    const { toast } = useToast();
    const queryClient = useQueryClient();
    const mutation = useMutation({
        mutationFn: (newId: string) => createPack(newId, name, description, userId),
