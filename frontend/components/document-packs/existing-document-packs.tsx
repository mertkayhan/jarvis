...
import {
  deletePack,
  ListDocumentPacksResp,
  listPacks,
} from "./document-packs-actions";
import { Skeleton } from "../ui/skeleton";
import { useRouter } from "next/navigation";

export function ExistingDocumentPacks() {
  ...
  const deleteMutation = useMutation(
    {
      ...
      onError: (error) => {
        console.error(error);
        toast({
          title: "Failed to delete question pack",
          variant: "destructive",
        });
      },
    },
        ...
        {data.packs.map((pack, i) => (
          ...
          >
            ...
            <Button
              ...
              type="button"
              key={i}
              onClick={() => router.push(`/document-repo/${pack.id}`)}
            >
              <svg
                className="w-4 h-4"
                viewBox="0 0 15 15"
