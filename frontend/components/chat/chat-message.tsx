...
export function ChatMessage({ message, streaming, setCurrentContext }: ChatMessageProps) {
    const imgs: string[] = message.data && JSON.parse(message.data)["images"] || []
    ...
    return (
        <div>
            ...
            <Dialog>
                ...
                <DialogContent
                    className='max-w-none w-full h-full flex flex-col items-center justify-center bg-background/50'
                >
...
export function ImgCarousel({ imgs, startIndex }: ImgCarouselProps) {
    return (
        <Carousel className="w-[5/6] h-[5/6] items-center justify-center" opts={{
            align: "start",
            loop: true,
...
export function AIMessage({ message, streaming, onCopy, isCopied, liked, setLike, setCurrentContext }: AIMessageProps) {
    return (
        <>
            <div
            ...
            <Tooltip>
                <TooltipTrigger asChild>
                    <div>
                        ...
                        >
                            <span className={`inline-block h-1.5 w-1.5 rounded-full ${badge.bgColor}`}></span>
                            {badge.text}
                        </span>
                    </div>
                </TooltipTrigger>
                <TooltipContent>
                    <p>This response received a faithfullness score of {Math.round(score)}%</p>
                </TooltipContent>
            </Tooltip>
        </TooltipProvider>
    )
