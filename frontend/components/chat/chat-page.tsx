...
function useDefaultPersonality(userId: string) {
  ...
  return {
    ...
    defaultPersonalityLoading: isLoading,
  };
}
...
export default function ChatPage({
  ...
  const { defaultPersonality, defaultPersonalityLoading } =
    useDefaultPersonality(user?.email as string);
  const [selectedPersonality, setSelectedPersonality] = useState<
    Personality | undefined
  >(undefined);
  const [chatGenerating, chatGeneratingDispatch] = useReducer<
    Reducer<Record<string, boolean>, any>
  >(chatGeneratingReducer, {});
    ...
    <div className="h-full flex flex-col flex-1">
      ...
      <div className="flex flex-col h-full">
        <div className="flex h-screen">
          ...
          <div className="flex w-full h-full overflow-auto pl-0 animate-in duration-300 ease-in-out peer-[[data-state=open]]:lg:pl-[250px] peer-[[data-state=open]]:xl:pl-[300px]">
            <Chat
              userId={user?.email as string}
              path={path}
              greeting={greeting}
